...
export default function PropertyDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => {
    const fetchProperty = async () => {
      try {
        const [propRes, reviewsRes] = await Promise.all([
        ...
        const propData: PropertyData = await propRes.json()
  ...
  useEffect(() => {
    ...
    fetch(`/api/wishlist/${property.id}`)
      .then((r) => r.json())
      .then((data) => setIsWishlisted(data.isWishlisted ?? false))
      .catch(console.error)
  ...
  const handleToggleWishlist = async () => {
    ...
    try {
      if (isInitializing) { toast.error("Please wait..."); return }
  ...
  const handleReserve = async () => {
    ...
    try {
      setSubmitting(true)
      await createBooking({ listingId: property.id, checkIn, checkOut, guests })
      setSuccess(true)
    ...
    return (
      <div className="min-h-screen bg-gradient-to-b from-stayfinder-cream to-white dark:from-indigo-950 dark:to-purple-950 flex items-center justify-center">
        <div className="text-center">
          <div className="animate-spin rounded-full h-12 w-12 border-b-2 border-stayfinder-forest mx-auto mb-4" />
          <p className="text-stayfinder-forest/80 dark:text-slate-300">Loading property...</p>
        </div>
      </div>
    )
  }
  ...
  if (submitting) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-white dark:bg-black">
        <Loader2 className="h-16 w-16 animate-spin text-stayfinder-forest mb-4" />
        <p className="text-lg text-stayfinder-forest dark:text-white">Creating your reservation...</p>
      </div>
    )
  }
  ...
  if (success) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-white dark:bg-black text-center">
        ...
        <h2 className="text-2xl font-bold text-stayfinder-forest dark:text-white">Booking created!</h2>
              ...
              <div className="flex items-center gap-4 text-sm text-muted-foreground">
                {avgRating && (
                  <div className="flex items-center">
                    ...
                    <span className="ml-1">({reviews.length} {reviews.length === 1 ? "review" : "reviews"})</span>
                  </div>
                )}
                <div className="flex items-center">
            ...
            <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-6">
              ...
              <TabsContent value="location" className="space-y-6">
                <Card className="p-6 border-stayfinder-sage/20">
                  ...
                  {hasCoordinates ? (
                    ...
                    <div className="h-32 flex items-center justify-center rounded-lg bg-stayfinder-sage/10 text-muted-foreground text-sm">
              ...
              <TabsContent value="reviews" className="space-y-6">
                <Card className="p-6 border-stayfinder-sage/20">
                  ...
                  {reviews.length === 0 ? (
                    <p className="text-muted-foreground text-sm">Be the first to review this property.</p>
                  ) : (
                  ...
                  <Popover>
                    ...
                    <PopoverContent className="w-auto p-0" align="start">
                      <Calendar
                        ...
                        disabled={(date) =>
                          date < new Date() || (checkIn ? date <= checkIn : false) || isDateBooked(date)
                        }
                        initialFocus
                      />
                    </PopoverContent>
